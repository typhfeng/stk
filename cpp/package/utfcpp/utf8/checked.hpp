...
namespace utf8
{
    ...
    {
        assert(utf8::internal::is_code_point_valid(cp));
    ...
    utfchar32_t next(octet_iterator& it, octet_iterator end)
    {
        ...
        assert(err_code == internal::UTF8_OK);
        return cp;
    }
    ...
    utfchar32_t prior(octet_iterator& it, octet_iterator start)
    {
        ...
        while (utf8::internal::is_trail(*(--it))) {
            assert(it != start); // error - no lead byte in the sequence
        }
    ...
    octet_iterator utf16to8 (u16bit_iterator start, u16bit_iterator end, octet_iterator result)
    {
        while (start != end) {
            ...
            if (utf8::internal::is_lead_surrogate(cp)) {
                ...
                assert(utf8::internal::is_trail_surrogate(trail_surrogate));
                cp = (cp << 10) + trail_surrogate + internal::SURROGATE_OFFSET;
            }
            // Lone trail surrogate
            else {
                assert(!utf8::internal::is_trail_surrogate(cp));
            }
    ...
    class iterator {
      ...
      {
          assert(range_start == rhs.range_start && range_end == rhs.range_end);
          return (it == rhs.it);
      }
      bool operator != (const iterator& rhs) const
      {
