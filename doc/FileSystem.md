...
对于m.2的机器, 用fstab持久化挂载, 对于usb测试机器, 每次需要手动挂载
记得每次用ssd前检察挂载状态, 不要把数据写到wsl的虚拟盘里
...
#==================================================================================================
# wsl2-ubuntu:
# 1) 找到这块盘的设备名，一般是 /dev/sda 或 /dev/sdb
lsblk -o NAME,MODEL,SIZE,ROTA,TRAN,FSTYPE,MOUNTPOINT
...
# 4) 格式化 XFS，带高级调优 (sudo apt install xfsprogs)
sudo mkfs.xfs -m bigtime=1 -n size=8192 -i size=512 -d agcount=256 $PART
# meta-data=$PART              isize=512    agcount=256, agsize=3815398 blks
